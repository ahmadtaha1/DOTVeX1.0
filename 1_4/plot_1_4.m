...
% Calls model_1_4.m to solve system and observer dynamics before plotting.
% 't' is time vector and 'z' is solution vector.
% Extracts actual and estimated state values.
% Plots system states, error dynamics norm, error dynamics, unknown input and disturbance/attack.
...
disp(' ');
disp('Solving ODE...')
[t, z] = ode15s(@model_1_4, tspan, [x0; xhat0], []); % solving ODE's through ode suit
disp('Done!')     
...
%Plot disturbances/attack
for k = 1:sys.dim.nv
    figure;
    plot(t, eval(sys.v{k}), 'linewidth', 2.5);
    grid on
    grid minor
    xlabel('Time (s)','interpreter','latex','FontSize',fs);
    ylabel(['$v_',num2str(k),'(t)$'],'interpreter','latex','FontSize',fs); 
    set(gca,'FontName','Times New Roman','fontsize',fs);
end

fs=24;
figure;
plot(t, nor, 'LineWidth', 3);
grid on
grid minor
xlabel('Time (s)','interpreter','latex','FontSize',fs);
